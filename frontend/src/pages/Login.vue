    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div>
          ...
          <input id="password" v-model="password" type="password" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 border" placeholder="••••••••" />
        </div>
...
const handleSubmit = async () => {
  ...
  // JS-level validation (cannot rely on HTML5 required alone)
  if (!email.value.trim()) {
  ...
  try {
    if (isRegistering.value) {
      try {
        await account.create(ID.unique(), email.value.trim(), password.value, name.value.trim())
      } catch (e: any) {
        if (e.code === 409) {
    ...
    // Clean any existing session before creating new one
    try {
